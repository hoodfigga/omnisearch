...
class PageExtractor:
    ...
    def extract_from_html(cls, html_content: str, page_url: str) -> List[ItemRecord]:
        ...
        # Parse the document exactly once; all sub-extractors share this soup.
        ...
        if FileHostExtractor.is_file_host_url(page_url):
            file_host_rec = FileHostExtractor.extract(html_content, page_url, soup=soup)
            if file_host_rec:
        ...
        if FileHostExtractor.is_open_directory(soup, page_url):
            dir_files = FileHostExtractor.extract_open_directory_files(html_content, page_url, soup=soup)
            if dir_files:
        ...
        json_ld_records = JsonLdExtractor.extract_from_html(soup, page_url)
        extracted_records.extend(json_ld_records)

        # 3. OpenGraph
        og_record = OpenGraphExtractor.extract_from_html(soup, page_url)
        if og_record:
            extracted_records.append(og_record)

        # 4. HTML Meta / Twitter Cards
        html_meta_record = HtmlMetaExtractor.extract_from_html(soup, page_url)
        if html_meta_record:
